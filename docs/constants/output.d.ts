export declare const buttonLabels: {
    readonly uploadExcel: "Upload Excel File";
    readonly transposeData: "Transpose Data";
    readonly calculateScores: "Calculate Scores";
    readonly exportDiagram: "Download Diagram for Group";
};
export declare const diagramInfo: {
    readonly title: "Teamwork Indicator";
    readonly subtitle: "Experts in Teamwork";
    readonly description: "This diagram shows your group's scores on four dimensions.\n\nThe farther out from the center your values are placed on each of the four axes, the better. In your group, discuss what the scores may indicate about the current functioning of the group.\n\nIf you have completed the Teamwork Indicator before, you can also compare the current values with the previous ones and discuss what any changes may say about the development in your group.\n";
};
export declare const radarDimensions: readonly [{
    readonly id: "honestAndDirect";
    readonly label: "Honest and direct";
    readonly description: "This dimension indicates the attitudes of the group members toward open, direct\nand honest sharing of feedback and reflections about the functioning of individuals,\nsubgroups and the whole group.";
}, {
    readonly id: "workCommitment";
    readonly label: "Work Commitment";
    readonly description: "This dimension indicates the degree of priority members give to the tasks of the group,\nwillingness to collaborate constructively and be committed in the contributions towards\ncompletion of the group tasks.";
}, {
    readonly id: "management";
    readonly label: "Management";
    readonly description: "This dimension indicates how well the group is organized with regard to the management of\ntime, progress and being goal-directed as well as productive.";
}, {
    readonly id: "socialCooperation";
    readonly label: "Social Cooperation";
    readonly description: "This dimension indicates the degree of positive social climate, support, the willingness\nto listen and to include the members of the group.";
}];
export type RadarDimensionId = (typeof radarDimensions)[number]['id'];
//# sourceMappingURL=output.d.ts.map